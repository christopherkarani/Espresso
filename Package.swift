...
// Optional GGUF / EdgeRunner integration. Enabled only when a local Edgerunner
// checkout is present so a clean clone of Espresso alone can resolve and build.
//
// Detection order:
//   1. ESPRESSO_EDGERUNNER_PATH (absolute or relative path to Edgerunner root)
//   2. ../Edgerunner (sibling checkout)
//
// Disable even when present: ESPRESSO_DISABLE_GGUF=1
private func resolveEdgerunnerPath() -> String? {
    if ProcessInfo.processInfo.environment["ESPRESSO_DISABLE_GGUF"] == "1" {
        return nil
    }
    var candidates: [String] = []
    if let envPath = ProcessInfo.processInfo.environment["ESPRESSO_EDGERUNNER_PATH"],
       !envPath.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
        candidates.append(envPath)
    }
    candidates.append("../Edgerunner")
    for candidate in candidates {
        let packageSwift = (candidate as NSString).appendingPathComponent("Package.swift")
...
let edgerunnerPath = resolveEdgerunnerPath()
let enableGGUF = edgerunnerPath != nil

var packageDependencies: [Package.Dependency] = []
if let edgerunnerPath {
    packageDependencies.append(.package(path: edgerunnerPath))
}
...
var packageProducts: [Product] = [
    ...
    .library(name: "ANETypes", targets: ["ANETypes"]),
    .library(name: "ANERuntime", targets: ["ANERuntime"]),
...
if enableGGUF {
    packageProducts.append(.executable(name: "espresso-gguf", targets: ["EspressoGGUFRunner"]))
    packageProducts.append(.library(name: "EspressoGGUF", targets: ["EspressoGGUF"]))
}
...
if enableGGUF {
    packageTargets.append(contentsOf: [
        ...
        .testTarget(
            ...
            dependencies: [
                "EspressoGGUF", "ModelSupport", "ANETypes", "RealModelInference",
                .product(name: "EdgeRunner", package: "Edgerunner"),
                .product(name: "EspressoEdgeRunner", package: "Edgerunner"),
            ],
