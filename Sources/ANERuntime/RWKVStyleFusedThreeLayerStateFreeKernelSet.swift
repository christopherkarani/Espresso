...
public struct RWKVStyleFusedThreeLayerStateFreeKernelSet: ~Copyable {
    public static let defaultLaneSpatial = 32

    internal enum KernelKind: String, CaseIterable {
        case fusedThreeLayerStateFreeStep
    }
    ...
    public init(
        ...
        guard laneSpatial > 0 else {
            throw .invalidArguments("fused three-layer state-free recurrent laneSpatial must be > 0")
        }
    ...
    private static func makeFusedStepSpec(
        ...
        let generator = RWKVStyleFusedThreeLayerStateFreeStepGenerator(laneSpatial: laneSpatial)

        return CompileSpec(
            kind: .fusedThreeLayerStateFreeStep,
